...
function gateFacts(gate: PolicyGateSummary | undefined, taskId: string): string {
  if (gate === undefined) {
    return `- Gate implement: not evaluated. Run \`visp-kit gate implement --task ${taskId}\` before coding.`;
  }
...
1. Run: \`visp-kit checklist update --task ${task.id} --item read-context --status done\`
   (after reading this prompt and the Facts above)
2. Implement only ${task.id} inside the allowed files.
3. Run: \`visp-kit checklist update --task ${task.id} --item implement-selected-task --status done\`
4. Confirm only allowed or expected files changed, then run: \`visp-kit checklist update --task ${task.id} --item scope-check --status done\`
5. Update or add tests, then run: \`visp-kit checklist update --task ${task.id} --item tests-updated --status done\`
...
7. Run: \`visp-kit done --task ${task.id} --input-tokens <n> --output-tokens <n>\`
   (no numeric usage available? run: \`visp-kit done --task ${task.id} --usage-unavailable --model <agent> --usage-note "<reason>"\`)
   - \`visp-kit done\` runs verify, review, reconcile, the checklist check, and \`visp-kit next\` in order.
   - A step FAILED -> fix only the reported issues, then rerun the same \`visp-kit done\` command.
8. Report the Next command printed by \`visp-kit done\` as your final status.

## If blocked

- Gate blocked: do NOT edit code. Run the next allowed command shown in Facts, then run \`visp-kit context ${task.id}\` to regenerate this prompt.
