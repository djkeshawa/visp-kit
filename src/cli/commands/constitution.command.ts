...
export function createConstitutionCommand(
  ...
  const runConstitution = dependencies.runConstitution ?? runConstitutionWorkflow;
  ...
  return new Command("constitution")
    ...
    .addOption(
      new Option("--preset <preset>", "Constitution preset.").choices(presetSchema.options)
    )
    .addOption(new Option("--budget <budget>", "Budget mode.").choices(budgetModeSchema.options))
    ...
    .action(async (targetPath: string | undefined, options: ConstitutionCommandOptions) => {
      const result = await runConstitution(workflowOptions(targetPath, options, dependencies.cwd));
