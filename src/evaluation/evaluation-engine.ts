...
export function evaluateProject(input: {
        ...
        checks.push(
          check({
            category:
              key === "context"
                ? "context"
                : key === "verification" || key === "review" || key === "reconcile"
                  ? "evidence"
                  : "workflow",
            severity:
              input.strict && ["spec", "plan", "taskGraph"].includes(key) ? "error" : "warning",
            title: `${label} missing`,
  ...
  if (state.taskGraph !== undefined) {
    for (const task of state.taskGraph.tasks) {
      ...
      if (
        ...
        /add|update|create|delete|validate|calculate|workflow/i.test(task.title)
      ) {
        checks.push(
          check({
            category: "traceability",
  ...
  if (state.contextPack?.overBudget) {
    checks.push(
      check({
        category: "budget",
        severity: "warning",
