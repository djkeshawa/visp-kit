...
export function reviewDocumentation(input: {
  ...
  const commandOrSchemaChanged = paths.some(
    (file) =>
      file.startsWith("src/cli/commands/") ||
      file.startsWith("src/artifacts/schemas/") ||
      file === "src/cli/main.ts"
  );
  const docsChanged = paths.some((file) => /^README|^docs\//i.test(file) || file.endsWith(".md"));
  ...
  return [
    finding({
      ...
      description:
        "The diff changes command or artifact-schema surfaces but does not include documentation updates.",
