...
describe("terminal formatting helpers", () => {
  ...
  it("formats key value lines", () => {
    expect(formatKeyValue("State", "PLAN_READY", noColor)).toBe("State: PLAN_READY");
  });

  it("formats normalized status labels", () => {
    expect(formatStatus("Needs Clarification", noColor)).toBe("[needs-clarification]");
  });

  it("formats calm severity lines", () => {
    expect(formatSuccess("Ready", noColor)).toBe("[ready] Ready");
    expect(formatWarning("Clarify scope", noColor)).toBe("[needs-clarification] Clarify scope");
    expect(formatError("Command failed", noColor)).toBe("[error] Command failed");
