...
// D-118 decision 4: the three-way migration rule. Byte-identical → skip;
// unmodified-but-superseded (disk matches the recorded generated hash) →
// regenerate silently; user-modified → refuse as stale, never overwrite.
describe("recordedHash migration rule (D-118)", () => {
  function textFile(root: string, contents: string): AgentPlannedFile {
    return {
      kind: "text",
      path: path.join(root, "generated.md"),
      contents
    } as AgentPlannedFile;
  }

  it("regenerates silently when disk still matches the recorded generated bytes", async () => {
    const root = await workspace();
    const oldContents = "old template with visp verify\n";
    await writeAgentPlannedFile(root, textFile(root, oldContents), {
      force: false,
      dryRun: false
    });

    const result = await writeAgentPlannedFile(root, textFile(root, "new template\n"), {
      force: false,
      dryRun: false,
      recordedHash: generatedContentHash(oldContents)
    });

    expect(result.ok).toBe(true);
    if (!result.ok) return;
    expect(result.value.action).toBe("updated");
    expect(await readFile(path.join(root, "generated.md"), "utf8")).toBe("new template\n");
  });

  it("refuses a user-modified file even when a recorded hash exists", async () => {
    const root = await workspace();
    const oldContents = "old template\n";
    await writeAgentPlannedFile(root, textFile(root, oldContents), {
      force: false,
      dryRun: false
    });
    // The user edits the generated file by hand.
    await writeFile(path.join(root, "generated.md"), "my hand-tuned version\n", "utf8");
    ...
    expect(await readFile(path.join(root, "generated.md"), "utf8")).toBe("my hand-tuned version\n");
  });
});
