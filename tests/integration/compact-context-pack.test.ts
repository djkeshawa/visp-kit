...
describe("the compact context pack", () => {
   ...
   *
   * This is the same shape as the cohort measurement (cap alone -49.49%
...
/**
 * THE INVARIANT THE -49% RESTS ON.
 *
 * Measured on 21 capability-eligible tasks, `balanced` mode, the compact
 * snippet cap against no cap with no graph, no store and no understanding case
 * on either side, on the SHIPPED binary: input tokens 14,539.048 -> 7,343.333
 * (-49.49%), and bodied file recall
 * 0.4382395382 on BOTH sides — the same floating-point number on the cohort and
 * on every task individually. Listed file recall, bodied precision, files
 * bodied and irrelevant bodied files were identical on every task too. The only
 * field that moved was input tokens.
 *
 * (Two superseded pairs. 16,709.607 -> 8,019.393, -52.01%, came from a visp-dev
 * measurement build, neither endpoint producible on the build that ships the
 * default it justified; the shipped re-run adds a flat ~292 tokens to both arms
 * and 0-7 to the capped one. 17,002.071 -> 8,317.000, -51.08%, is this same
 * shipped-binary run on the 28-record cohort, before seven author-dependent
 * records were reclassified out — on that cohort symbol recall moved too, on 2
 * tasks in the repository those seven came from. Every non-token metric is
 * unchanged under both restatements, which is why the invariant below is
 * unaffected.)
 *
...
describe("the compact snippet cap with no understanding case", () => {
   ...
   * produced -49.49% carried both of those sections; a cap that also dropped
