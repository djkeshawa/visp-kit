...
The current implemented boundary includes WorkflowAction 2.0, 3.0, 3.1, and 3.2 plus
...
P3-08 likewise preserves 3.1 and adds assurance-aware WorkflowAction 3.2.
...
2. WorkflowAction 3.0, 3.1, and 3.2 are additive and explicitly requested; none
...
## Implemented WorkflowAction 3.2 projection

WorkflowAction 3.2 preserves 3.1 and advances the canonical action to version
`1.2`. Its domain-separated identity uses
`visp.workflow-action\0canonical-1.2\0`. It adds a closed `assuranceSummary`
that either identifies a schema-valid assurance case by normalized path, exact
raw content hash, independent case hash, verdict, sorted mandatory hotspots,
and Kit-evaluated review-decision state, or reports an honest unavailable
reason. Review status uses Kit's current, missing, rejected, stale, and invalid
states and preserves a nullable decision hash explicitly.

The assurance case keeps the predecessor 3.1 action identity used when it was
generated; 3.2 hashes the resulting assurance summary and does not rewrite the
case. The summary never adds a competing command. The action's top-level
`nextCommand` remains the sole Kit-authoritative command.
...
- `urn:visp:schema:workflow-action:3.1`; and
- `urn:visp:schema:workflow-action:3.2`.
...
Kit accepts exact `visp next --format json --protocol 2.0`, `3.0`, `3.1`, or `3.2`.
